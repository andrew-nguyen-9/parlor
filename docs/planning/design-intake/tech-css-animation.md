...
- [x] C) Case-by-case — decide per room in the matrix below (Q2)
...
|------|--------------------------------|-------|--------------|-------------------------------|
| clock (Chronos) | `clock-swing` pendulum / gear-train canvas |x| | |
| streak (Ignite) | `flame-dance` + Phaser candle |x| | |
| map (Atlas) | starfield canvas drift |x| | |
| mystery (Sanctum) | `eye-glow` seal pulse |x| | |
| wedges (Fractures) | shattered-mirror ring fill |x| | |
| thread (Thread) | chain-links reveal (finite, not a loop) |x| | |
| seance (Seance) | `flicker` candle |x| | |
...
- [x] B) Home / lobby only; game rooms drop it so the signature loop is unambiguous
- [x] C) Only the wildcard rooms (gauntlet/seance) that have no strong diegetic loop
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Every room themes its own entrance (most bespoke, highest budget)
- [ ] Other: __________________________________________
> USER NOTES (per game):
...
- [x] B) Staggered per-cell entrance (transform+opacity, ≤500ms total, capped stagger)
...
- [x] B) Opacity + short slide/scale in (transform, ≤350ms) with entrance easing
- [x] C) Themed per room (chain "stitch", rung "climb", spirit "manifest") — bespoke keyframes
- [ ] Other: __________________________________________
> USER NOTES (per game):
...
- [x] A) One shared utility (scale 0.97 + border/shadow shift, 120ms) reused everywhere
- [x] B) Shared press, but correct/wrong get a room-tinted flash on top
- [x] C) Per-room press character (e.g. board cell "flip-lift", wedge "shard-snap")
- [ ] Other: __________________________________________
> USER NOTES (per game):
...
| thread | | |
| seance | | |
- [x] A) None get shake — wrong = static color/glyph change only (calmest)
- [x] B) A single shared ≤250ms shake/flash utility, opt-in per room above
- [ ] Other: __________________________________________
> USER NOTES:

### Q9: Correct-answer / score-up feedback (points tick, wedge fills, streak increments). CSS budget?
- [x] A) Number/pill transitions only (color + count, ≤300ms), no particle-y flourish
- [x] B) A short one-shot "pop" keyframe (scale bounce, ≤400ms) on the earned element
- [x] C) Room-diegetic (wedge fills its slice, streak flame brightens via `--flame`) — no generic pop
- [ ] Other: __________________________________________
> USER NOTES (per game):

### Q10: Overlay / results / deck-zoom enter+exit (Esc + backdrop close, focus-trapped). Transition?
- [x] A) Backdrop fade + panel scale-from-95% (≤300ms), shared across all overlays
- [x] B) Fade only — no scale (fastest, least layout thrash)
- [x] C) Per-context (results "curtain", zoom "lift") bespoke
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Idle allowed but must be the *same* element as the signature (e.g. a glow that also is the loop)
- [ ] Other: __________________________________________
> USER NOTES (per game):
...
- [x] B) One hero title per room may get a single slow sheen sweep on mount (finite, once)
- [ ] Other: __________________________________________
> USER NOTES (per game):
...
- [x] B) Yes, map (Atlas) — a subtle cursor parallax on the starfield chrome
...
- [x] B) Each room ships a hand-tuned still (like streak's `opacity:0.7`) — more polish
- [ ] Other: __________________________________________
> USER NOTES (per game):
...
- [x] B) A `selftest`/lint assertion: every `@keyframes` name appears in the reduced-motion block
...
- [x] A) Bands are hard caps — no room exceeds 500ms for any finite transition
...
- [x] B) Chrome gets micro-transitions only (press/hover/reveal), no loops
...
- [x] B) Give each a *modest* signature loop to match the immersive rooms' richness
...
- [x] C) **Lively** — spend the budget; every room should feel alive at idle
- [ ] Other: __________________________________________
> USER NOTES:
