...
- [x] A) All 7 games + all 4 orphan rooms (bed everywhere)
...
- [x] A) Yes — one quiet lobby bed, distinct from any room bed
...
- [x] B) Ultra-minimal bed (a single low pad, no rhythm) so it never competes with reaction timing
...
- [x] A) Sample every bedded room (source/commit `ambient-<room>.mp3` for all)
...
- [x] B) Vary timbre per room too (waveform, filter cutoff, LFO rate) for distinct moods
...
- [x] A) The procedural drone (status quo — auto-swap on load, seamless)
...
- clock (Chronos): tense-pulse
- streak (Ignite): warm-drone
- map (Atlas): sparse-ambient
- mystery (Sanctum): knives-out
- wedges (Fractures): tense-pulse
- thread (Thread): airy-open
- seance (Seance): dark-ritual
- board / daily / gauntlet / ladder: playful-arcade
...
- [x] B) A subtle pulse only where it fits the game (e.g. clock/Chronos ticking, streak/Ignite heartbeat)
...
- [x] A) Yes — bed root, SFX, and stinger share a key per room (harmonically unified)
...
- [x] B) Duck the bed briefly (lower its gain) so the SFX/stinger reads clearly, then restore
...
- [x] C) Shift on outcome (a darker layer on a miss, brighter on a hit)
- [x] D) Reactive only in the games where it's dramatic; static elsewhere
- [ ] Other: __________________________________________
...
- [x] C) Brief silence between rooms (a "breath"), then the new bed
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] A) Keep beds well under SFX — atmosphere you barely notice (status quo)
...
- [x] C) Add a bed on/off toggle (distinct from full mute) but no continuous slider
- [ ] Other: __________________________________________
> USER NOTES:

### Q15: Per-game mute memory — should muting be global or remembered per room?
- [x] A) Global — one mute state across the whole app (status quo)
...
- [x] A) On by default (respect autoplay rules: start on first gesture)
...
- [x] A) OpenGameArt.org CC0 drone/pad loops (recommended in the audio done-note)
- [x] B) Freesound.org CC0-only filter
...
- [x] A) Short (~10–20s) loop, equal-power crossfade at the seam
- [ ] B) Long (~60–120s) so repetition is less obvious (bigger asset weight)
- [x] C) Don't sample — the procedural drone is seamless by construction; use it wherever seams are a risk
- [ ] Other: __________________________________________
> USER NOTES:

### Q19: Asset-weight budget — how much bundle size are beds allowed to add? (offline/committed repo)
- [x] A) Tight — procedural default, sample at most 1–2 signature rooms
...
- [x] B) Alias the new rooms onto existing keys (e.g. mystery→board, thread→wedges) to reuse tuning
