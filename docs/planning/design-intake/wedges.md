...
- [ ] B) An intact mirror that visibly shatters into the six wedges as the round begins (one-time break animation)
...
- [ ] B) Companion — mirror on one side (or above), question card is the main focus beside it
- [ ] C) Ambient — mirror is a faint full-bleed background, question card sits clearly on top
- [ ] Other: __________________________________________
> USER NOTES:

### Q: The six wedges map to the six categories (Science, History, Entertainment, Sports, Art, Geography). How are they arranged?
- [ ] A) Fixed clock positions — each category always the same wedge, so regulars learn the layout
- [ ] B) Daily-shuffled — wedge positions rotate with the daily seed (fresh mirror each day)
- [ ] C) Fixed positions, but the fault-line crack pattern between them shifts daily
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] C) A one-line "the mirror is broken again" prompt over the idle glass
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Should the `WedgeLegend` chip strip appear at setup, or only once play starts?
- [ ] A) Present from setup — all six chips dim, previewing the categories to come
- [ ] B) Hidden until the first question, then slides in
- [ ] C) Fold the legend into the mirror itself (no separate chip strip); each wedge is its own status
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) Correct = that category's wedge reassembles and its fault lines seal with silver veins; miss = a new crack spreads
- [ ] B) Correct = the wedge brightens/clears (stays cracked but lit); full repair only at wedge-earned
- [ ] C) Correct = a single shard glides back into its wedge; the wedge needs several to fully reform
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) Earned wedge goes solid/mirror-finish and stops flickering — visibly "done"
- [ ] B) A silver seam locks around the earned wedge's border
- [ ] C) The earned category's `WedgeLegend` chip fills + checks; the wedge just brightens
- [ ] Other: __________________________________________
> USER NOTES:

### Q: How does the player choose an answer (the choice buttons)?
- [ ] A) Frosted-crystal buttons stacked below the mirror (current-style list, glass skin)
- [ ] B) Answer options embedded as shards around the mirror frame — tap the shard
- [ ] C) List of glass buttons, but the active wedge's accent color tints them
- [ ] Other: __________________________________________
> USER NOTES:

### Q: The current active category — should the player know which wedge they're answering for before they read the question?
- [ ] A) Yes — the active wedge pulses (`•`) and the question card is tinted its accent color
...
- [ ] A) Nothing — tap an answer or let the timer run out, that's the whole verb (keep it pure)
...
- [ ] A) As earned wedges only — the mirror IS the score; number of reformed wedges = your result
- [ ] B) Wedge count reflected inside the mirror's center (per spec's "score reflected inside the mirror")
- [ ] C) Keep a small numeric HUD too, but styled as an etched engraving, not a flat counter
- [ ] Other: __________________________________________
> USER NOTES:

### Q: The speed bonus (`100 + 20×secondsLeft`) currently flashes a "+N". How should that feedback read in the glass theme?
- [ ] A) A crystalline "+N" that refracts/sparkles off the just-repaired wedge, then fades
...
- [ ] A) A hairline crack creeping across the active wedge — reaches the edge = time's up
...
- [ ] C) Speed-weighted — 6 wedges AND high bonus points (a fast full clear) is the flex
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] C) Cracks accumulate visibly across the whole round, tallying your misses at the end
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) Full ceremony per spec: shards lift, rotate, mirror reforms into one pristine reflection, moonlight fills the screen
...
- [ ] A) The mirror settles half-reformed — earned wedges clear, unearned ones stay fractured (honest, a little melancholy)
- [ ] B) A gentle "the mirror rests unfinished until tomorrow" close over the partial glass
...
- [ ] A) No lose — you always finish with 0–6 wedges; the mirror just reflects how far you got
...
- [ ] C) Cinematic — slow, film-like reassembly with a musical swell
- [ ] Other: __________________________________________
> USER NOTES:

### Q: The reformed mirror shows "one perfect reflection." What is reflected?
- [ ] A) An abstract pristine silver surface — clean glass, no imagery
- [ ] B) The player's score/streak elegantly engraved into the clear glass
- [ ] C) A subtle rendering of today's categories/answers as an etched summary
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) A distinct beat — smoke rolls across, mirror dims, then bonus questions surface as "final shards"
- [ ] B) Seamless — bonus questions just continue, no visible phase break
- [ ] C) A short interstitial naming the bonus round ("the last fractures")
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] C) Auto-advance, but the repair animation length sets the pace (correct = longer, satisfying beat)
- [ ] Other: __________________________________________
> USER NOTES:

### Q: How should retries / replays work for the daily?
- [ ] A) One shot per day — the daily mirror can only be broken once (current daily-game norm)
...
- [ ] A) Daily = the "real" antique mirror with the dated silver frame; freeplay = a plainer practice pane
...
- [ ] B) Keep the Ghost as a side character (current framing), just glass-skinned
...
- [ ] A) Cumulative — the mirror carries all repairs and all cracks forward; final state = the whole story
...
- [ ] A) No — difficulty is invisible; the skin never signals "this one's hard"
...
- [ ] B) Keep 15s, but bonus-round questions feel more urgent purely via visuals (faster smoke, tighter crack)
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Category lockout means late-round questions come from fewer categories. Should that narrowing be surfaced?
- [ ] A) Yes — earned/locked wedges visibly "close," so it's clear the pool is shrinking to what's left
...
- [ ] B) No modes — one canonical experience
...
- [ ] B) Attainable most days for engaged players — the payoff is the ritual, not the rarity
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) Cold moonlit — obsidian + mirror silver + moonlight cyan, restrained and premium
...
- [ ] A) Lush — volumetric smoke, floating glass dust, visible light rays; cinematic (perf-gated)
...
- [ ] A) Etched line-art icons per category with distinct accent tints + per-wedge glass texture
- [ ] B) Color-only — each wedge is a distinct silvered tint, no iconography
- [ ] C) Icons + a signature refraction/highlight animation unique to each category on hover
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Typography direction (spec: elegant modern serif headings, editorial feel)?
- [ ] A) Serif headings + clean sans body — luxury editorial contrast
...
- [ ] A) SVG hairline masks (current `ShatteredMirror` approach), animated to spread/seal
- [ ] B) Richer refraction/caustic effects along cracks (GPU, perf-gated)
...
- [ ] A) Full dynamic bed — the whole interface brightens toward moonlit clarity as wedges fill
...
- [ ] A) Crystalline chimes on repair, glass resonance on miss, a low mirror-room drone bed (per spec, incl. faint frame creaks)
...
- [ ] A) Engraved into the silver frame around the mirror
- [ ] B) Reflected faintly in the mirror's center, revealed as it clears
...
- [ ] A) Freeze all ambient motion; repairs/misses become instant static state changes (crack appears/seals, no travel)
- [ ] B) Keep slow essential feedback (wedge fills) but kill particles, drifting smoke, and shimmer
- [ ] C) Offer a settings toggle in addition to honoring the OS preference
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Low-power / weak-GPU devices — how does the skin degrade if volumetric smoke and refraction are too heavy?
- [ ] A) Tiered fallback — drop particles → drop blur/refraction → flat glass gradients, auto-detected
- [ ] B) A single lightweight CSS-only tier that always runs (no WebGL/canvas dependency at all)
- [ ] C) Render the full effect; accept lower frame rates on old devices
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Offline / zero-env (PARLOR must play fully from the seed bank). Does the Fractures skin need anything network-bound?
- [ ] A) Fully offline — all glass/smoke/crack assets are inline SVG/CSS/canvas, zero fetches (required baseline)
...
- [ ] A) End the round gracefully — reform what was earned, leave the rest fractured (no soft-lock)
- [ ] B) Backfill from the bonus pool so the ring can still theoretically complete
- [ ] C) Show a clear "no more shards for this category today" state on the affected wedge
- [ ] Other: __________________________________________
> USER NOTES:

### Q: A player tabs away mid-round (timer running) or the tab loses focus — what's the fair behavior?
- [ ] A) Pause the timer + freeze the atmosphere while hidden; resume on return
...
- [ ] B) Timeout always wins at 0 — late taps are ignored
- [ ] C) Debounce with a tiny grace window so a near-deadline tap still registers
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Empty / error states (no daily set loaded, seed fallback, or a malformed question) — how does the mirror present that?
- [ ] A) An intact, dim, un-fractured mirror with a quiet "no reflection today" message (never a broken-looking error)
