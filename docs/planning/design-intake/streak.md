...
- [x] C) The full inscription (cipher runes) shown lit but unreadable; tray + clues below
...
- [x] B) Shuffled tray so alphabet order isn't a free clue
...
- [x] B) Framed by the shrine (roots, moss, candles) but puzzle stays center-stage
...
- [x] A) Rotates silently by date across sets (Elder Futhark, Anglo-Saxon, …)
...
- [x] B) Candles always visible as a progress meter (lit-per-rune-bound), no logic yet
- [x] C) Ship the full ritual (candles + compounds + propagation) as the baseline
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Both — drag for pointer, tap-tap for keyboard/AT parity
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Allow temporary duplicates; only flag them at Read time
- [ ] Other: __________________________________________
> USER NOTES:

### Q: How does "Read the runes" (the check action) gate?
- [ ] A) Only enabled once every rune is bound (filled == K, current)
- [x] B) Always tappable; partial reads reveal which bound runes are wrong
...
- [x] C) Keep all six, and add compound/element clues at higher tiers
- [ ] Other: __________________________________________
> USER NOTES:

### Q: How do players interact with clues?
- [x] A) Hover/tap a clue to ring-highlight the runes it names (current litGlyphs)
...
- [x] A) Place a compound on a rune to unlock/redirect a fire path (new verb)
- [ ] B) Compounds are passive clue-givers only — no placement
- [ ] C) Mix two compounds in a crucible to reveal a letter
- [ ] Other: __________________________________________
> USER NOTES: There should be more emphasis on the chemistry portion. I was imagining using runes on real chemical compound shapes and solving by creating the right compounds using the runes.

## Scoring — points, streaks, penalties, what "good" means

### Q: What is the headline score for a solve?
- [x] A) Misreads (wrong Reads) — fewer is better, "flawless" at zero (current share)
- [x] B) Time-to-solve
...
- [x] B) Cost a candle each — run out and the ritual fails
...
- [x] A) Yes — consecutive daily solves build a flame streak, shown on the home card
...
- [x] A) Yes — each hint (flicker → glow → outline) dims the flawless badge
...
- [x] B) Add a candle/flame emoji bar encoding misreads
- [x] C) Spoiler-safe grid only (no incantation, no rune identities)
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Reveal the incantation AND identify the day's featured compound
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Is there a real lose/fail state, or only "keep trying"?
- [ ] A) No lose — unlimited Reads, you always eventually solve (current)
- [x] B) Limited candles/Reads; running out ends the day as a soft loss
- [ ] C) No loss, but a "give up → reveal solution" option that voids the streak
- [ ] Other: __________________________________________
> USER NOTES: include a "give up → reveal solution" option that voids the streak
...
- [x] C) Full shrine wakes — roots pulse, embers rise, camera pulls up (v4 vision)
- [ ] Other: __________________________________________
> USER NOTES:

### Q: What does the end-of-round panel offer?
- [x] A) Share + "inscribe again" reset (current)
- [x] B) Add the day's educational card (compound / rune lore → Compendium)
...
- [x] C) One puzzle per day, hard-locked until tomorrow
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] A) Free, untimed — bind, re-bind, Read whenever (current, no timers per spec)
...
- [x] B) Wrong Read clears all bindings the clues can't prove — fresh-ish start
- [x] C) Wrong Read locks proven-correct runes, frees the rest
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Daily vs freeplay — what's the offering?
- [x] A) One deterministic daily ritual for everyone; that's it
...
- [x] A) Yes — longer inscriptions + more clue types as the week climbs to Sunday's Master Ritual
...
- [x] C) Interleaved — some runes only bind once their candle's compound is placed
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Both length and clue count scale together by weekday tier
- [ ] Other: __________________________________________
> USER NOTES:

### Q: How generous should the clue set be?
- [x] A) Always exactly-minimal (uniquely solving, no slack — current guarantee)
...
- [x] B) Incantations that are near-anagrams so an early wrong lock feels right
- [x] C) Compound red-herrings that look catalytic but aren't
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Should the incantation vocabulary itself scale difficulty?
- [x] A) Common words early, rarer/archaic words (harder to guess-ahead) later
- [ ] B) Keep vocabulary fixed; difficulty is purely structural (clues/length)
- [x] C) Theme words to the day's compound (e.g. "CINDER", "SULFUR")
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Any accessibility-driven easy mode?
- [ ] A) A "guided" toggle that auto-applies each clue's forced letter
- [x] B) A larger-tile / high-contrast rune mode, same puzzle
...
- [x] A) Deep charcoal + gold + burnished copper + amber firelight (v4 spec)
- [ ] B) Current PARLOR "history" amber (`#e0871f`) accent on surface tokens
- [x] C) Dark emerald moss + parchment, fire as the only warm accent
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Full GPU flame — flicker, heat distortion, spark showers (v4 aspiration)
- [ ] Other: __________________________________________
> USER NOTES:

### Q: How are runes rendered?
- [x] A) Unicode glyph + transliteration label, no bundled SVG (offline-robust, current)
- [ ] B) Hand-drawn SVG runes carved into stone, transliteration on hover
- [x] C) Unicode primary, optional SVG upgrade when assets load
- [ ] Other: __________________________________________
> USER NOTES:

### Q: What's the layout on desktop vs mobile?
- [x] A) Inscription top, rune grid center, letter tray bottom, clues collapsible
...
- [x] B) Sequential candle ignition down the inscription, ~2s
- [x] C) Full shrine-wakes set piece with camera move (v4)
- [ ] Other: __________________________________________
> USER NOTES:

### Q: What's the ambient mood while idle?
- [ ] A) Breathing flame + floating ash + ritual/fire audio bed (current)
- [x] B) Add fireflies, drifting mist, glowing fungi (v4 idle list)
- [ ] C) Near-silent, still — atmosphere only on interaction
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Does the Compendium ship as a visible surface?
- [x] A) Yes — a collectible archive of runes/compounds/lore filled by solving (v4)
...
- [x] A) Freeze the ember field to a still frame, no camera moves (current intent)
- [x] B) Also cut the win stinger / audio bed
- [x] C) Swap all motion for instant state changes; keep audio
- [ ] Other: __________________________________________
> USER NOTES:

### Q: If Phaser fails to load (or WebGL is blocked), what renders?
- [x] A) DOM fallback board — rune buttons + tray + Read, fully playable (required)
- [ ] B) Static rune grid image + "open in a supported browser" note
- [x] C) Canvas 2D fallback scene without shaders
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Auto-switch to a Latin-safe rune set when tofu is detected
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Archive-play of a date that was never generated (DB up, no row)?
- [x] A) "No inscription survives from that night" dark state (current)
...
- [x] C) Both: seed-bank first, inline generator as fallback
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) After N misreads, surface a "reveal one correct rune" mercy option
- [ ] Other: __________________________________________
> USER NOTES:

### Q: Clipboard blocked when sharing?
- [ ] A) Silently no-op (current)
- [x] B) Fall back to a selectable text box / native share sheet
- [x] C) Show a "copy failed" toast
- [ ] Other: __________________________________________
> USER NOTES:
