...
- [ ] B) A closed folder / wax-sealed dossier the player "opens" to begin (one tap to reveal)
...
- [ ] C) All three shown, but Hour stays dimmed until at least one timeline clue is read
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) A short 2–3 sentence scene-setter (manor, weather, who found the body)
...
- [ ] C) Collapsed to headers with counts; player expands each layer deliberately
- [ ] Other: __________________________________________
> USER NOTES:

### Q1.5: What seeds the daily case cast size (suspects × scenes × hours)?
- [ ] A) Fixed dimensions every day (e.g. 4×4×4)
- [ ] B) Grows across the week per the Mon→Sun ramp (small Monday cast, large Sunday household)
...
- [ ] B) Yes — swap in a themed surface per weekday while keeping the same deduction skill
- [ ] C) Board stays, but an optional secondary tool appears when the case warrants it
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] C) Collapse to two states only: *cleared* vs *prime*
- [ ] Other: __________________________________________
> USER NOTES:

### Q2.2: How does the player advance a pill vs. step it back?
- [ ] A) Tap row/pill = forward; long-press or right-click = back (current)
- [ ] B) Tap = forward; a small reverse arrow on the pill = back (no long-press)
- [ ] C) Tap cycles forward only; a "reset axis" button clears mistakes
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Allow multiple primes; accusation just needs the correct one selected
...
- [ ] C) Offer an opt-in "auto-eliminate" helper that clears logically-forced pills
- [ ] Other: __________________________________________
> USER NOTES:

### Q2.5: How should the Truth & Deception layer surface (reliable vs. lying witnesses)?
- [ ] A) Clue text names the reliability inline ("the butler never lies")
- [ ] B) A per-witness credibility badge on movement/testimony clues
...
- [ ] B) Add multi-select filter chips (combine Suspects + Hour)
- [ ] C) Add a "hide cleared" toggle so resolved evidence collapses away
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Partial credit shown on the share grid (🟩/🟥 per axis, as today) but still a loss
...
- [ ] C) Solved fast + with few clues consulted (efficiency flex)
- [ ] Other: __________________________________________
> USER NOTES:

### Q3.5: Should consulting fewer clues or reading fewer layers be rewarded?
- [ ] A) No — clue reading is free, never penalized
- [ ] B) Track clues opened; a "deduced from N clues" flex on the verdict
...
- [ ] B) A solve streak counter like the other Parlor rooms
...
- [ ] A) Primed suspect + scene + hour all equal the solution (current)
...
- [ ] A) Reveal the true solution immediately and lock the day (current)
...
- [ ] B) Bigger moment — ink-spread animation across the board, wax seal breaks
...
- [ ] B) Board fades; Case File collapses to a solved summary
- [ ] C) Board stays fully interactive as a read-only review of your reasoning
- [ ] Other: __________________________________________
> USER NOTES:

### Q4.5: What does the shareable result reveal vs. conceal?
- [ ] A) Per-axis 🟩/🟥 + clue count, no spoilers (current)
...
- [ ] B) Soft timer shown for the efficiency flex, no failure on timeout
- [ ] C) Optional "speedrun" mode toggled by the player
- [ ] Other: __________________________________________
> USER NOTES:

### Q5.2: Daily vs. free-play — what's offered?
- [ ] A) One deterministic daily case only (current)
...
- [ ] A) Yes — tags + verdict restore from localStorage per date (current)
...
- [ ] B) Layers unlock as the player tags pills (staged reveal)
...
- [ ] A) The single microlabel hint only ("tap a pill to mark…") (current)
- [ ] B) A dismissible first-run tutorial overlay
- [ ] C) An inline worked example on Monday's easy case
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Swipe between Case File / Board / Accusation as bottom-sheets (per v4 mobile spec)
- [ ] C) Sticky suspect summary + collapsible Case File
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) Follow the spec ramp: Mon small cast/single contradiction → Sun flagship household
...
- [ ] A) Cast size (more suspects × scenes × hours = larger grid)
- [ ] B) Clue count and how many are contradictions vs. plain facts
- [ ] C) Reliability noise — more lying/misremembering witnesses
- [ ] Other: __________________________________________
> USER NOTES:

### Q6.3: How should contradictions (Thursday's "evidence contradicts testimony") be tuned?
- [ ] A) At most one contradiction chain early week, more later
- [ ] B) Always solvable by elimination alone, contradictions optional shortcuts
- [ ] C) Contradictions are load-bearing — must be resolved to reach the unique solution
- [ ] Other: __________________________________________
> USER NOTES:

### Q6.4: Should the minimum-clue-path (how tight the logic is) be guaranteed?
- [ ] A) Always exactly one solution, no redundant clues
- [ ] B) One solution, with a few redundant confirming clues for reassurance
- [ ] C) One solution, and never solvable by guessing before all axes are constrained
- [ ] Other: __________________________________________
> USER NOTES:

### Q6.5: Any assist / hint affordance for stuck players?
- [ ] A) None — pure deduction, no hints (current)
- [ ] B) An optional "highlight a contradiction" nudge that costs score
...
- [ ] A) Dark walnut + brass + candlelight (current `--c-candle`, history hex, gold accents)
- [ ] B) Cooler marble + ink + midnight blue for a colder crime feel
- [ ] C) Occult tilt — deep oxblood, tarnished silver, faint arcane glow
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Add subtle candle-flicker on the glow
...
- [ ] B) Skin panels as physical evidence folders / pinned correspondence per v4
...
- [ ] A) Keep the soft audio note only
- [ ] B) Add a wax-seal-breaks visual on "prime", ink-spread on a correct final reveal
- [ ] C) Paper-lift on hover + a subtle stamp on clear
- [ ] Other: __________________________________________
> USER NOTES:

### Q7.5: How should the evidence layers read visually (Timeline / Movements & Testimony / Alibis Cleared)?
- [ ] A) Current: microlabel headers + hint text, numbered clue cards
- [ ] B) Distinct visual treatment per layer (timeline rail, testimony quote-cards, struck-through alibis)
- [ ] C) Layers as tabbed sections rather than a scroll
- [ ] Other: __________________________________________
> USER NOTES:

### Q7.6: What is the overall mood target?
- [ ] A) Knives Out / Clue — witty, warm, cozy-luxe
...
- [ ] A) Current display-serif title + gold moon ornament, understated
- [ ] B) A larger illustrated case-title plate (wax seal, date, weekday tone)
- [ ] C) Minimal — small title, let the board dominate
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) Freeze all atmosphere (glow/grain/dust) via each primitive's own contract (current)
- [ ] B) Also suppress the win ink-spread / seal-break animations
- [ ] C) Keep functional micro-feedback, drop only decorative loops
- [ ] Other: __________________________________________
> USER NOTES:

### Q8.2: Behavior when audio is muted or blocked (ambient + sfx + stinger)?
- [ ] A) Silently self-silence, game fully playable (current f1-audio contract)
- [ ] B) Show a small muted indicator so the player knows sound exists
- [ ] C) Provide a visual equivalent for the win stinger
- [ ] Other: __________________________________________
> USER NOTES:

### Q8.3: What happens when localStorage is unavailable (private mode / quota)?
- [ ] A) Keep playing in-memory, no error (current try/catch)
- [ ] B) Warn once that progress won't be saved
...
- [ ] A) Current line: "No clues match — try a different search or filter."
- [ ] B) Also surface a one-tap "clear filters" affordance
- [ ] C) Keep the message but grey out empty filter chips
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) A themed "the archive is sealed tonight" holding screen
...
- [ ] A) Disable "Make the accusation" until all three axes primed (current)
...
- [ ] A) Allow it — the game only judges against the solution, not consistency (current)
