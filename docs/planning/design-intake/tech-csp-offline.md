...
- [ ] A) One static CSP header in `next.config.mjs` `headers()` for all routes
...
- [ ] C) Allow `'unsafe-inline'`/`'unsafe-eval'` ONLY on the WebGL/Phaser routes, strict elsewhere
- [ ] Other: __________________________________________
...
- [ ] A) `'self' 'unsafe-inline'` globally (pragmatic — inline styles are unavoidable with the current stack)
...
|------|-------------------------------|----------------------------|
| clock (Chronos) | event/photo thumbnails? | ____ |
| streak (Ignite) | comparison item images? | ____ |
| map (Atlas) | flags (flagcdn), raster basemap (wikimedia) | ____ |
| mystery (Sanctum) | TMDB posters / stills | ____ |
| wedges (Fractures) | category art? | ____ |
| thread (Thread) | — | ____ |
| seance (Seance) | — | ____ |
| board/daily/gauntlet/ladder | mixed clue media | ____ |
- [ ] A) `img-src 'self' data:` + the exact 5 allowlisted hosts, everywhere
...
- [ ] B) Reflow to text-only question, no image slot at all
...
- [ ] B) Yes for the nightly-forged bank — snapshot images into `public/` at forge time, serve `'self'`
...
|------|------------------------------|----------------------|
| map (Atlas) | world-atlas polygons + `star-catalog.json` | ____ |
| clock (Chronos) | seed bank only | ____ |
| streak (Ignite) | seed bank only | ____ |
| mystery (Sanctum) | seed bank (+ any locale art?) | ____ |
| seance (Seance) | seed bank only | ____ |
| overture / home | `mansion-map.jpg`, `overture-melodies.json` | ____ |
- [ ] A) The current bundle is exactly right — no game needs more
- [ ] B) Add a specific per-game bundled asset (name it in notes)
- [ ] Other: __________________________________________
> USER NOTES (which asset, which game):
...
- [ ] B) SW that precaches the app shell + `seed-questions.json` so all rooms work fully offline after first visit
- [ ] C) SW scoped to the asset-heavy rooms only (Atlas polygons/star-catalog, overture melodies)
- [ ] Other: __________________________________________
...
- [ ] A) `font-src 'self'` only — all fonts self-hosted at build, no runtime font CDN (matches current)
...
- [ ] A) `media-src 'self'` everywhere — all audio bundled, no exceptions
- [ ] B) `'self' data:` (for any base64/procedural-synth blobs)
...
- [ ] A) No embeds anywhere — `frame-src 'none'`, `object-src 'none'` (strictest, matches current design)
...
- [ ] A) `'self'` only — all data server-fetched; client never calls out (map probe uses `<img>`, not fetch)
- [ ] B) `'self'` + the Neon serverless host (if any client-side query exists)
...
- [ ] A) `Content-Security-Policy-Report-Only` first, watch violations, then flip to enforcing
...
- [ ] A) Silent degrade to the bundled floor frame — no console error surfaced to player
- [ ] B) Silent degrade + a dev-only warning (only in `NODE_ENV=development`)
- [ ] C) Per game: Atlas silently drops the raster; Sanctum swaps to placeholder art; others unaffected
- [ ] Other: __________________________________________
> USER NOTES (per game):

### Q19. Other CSP/offline security headers to ship alongside (integration-wide, not per-game)?
- [ ] A) Add `Strict-Transport-Security`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `frame-ancestors 'none'`
- [ ] B) Just the CSP for now, add the rest later
- [ ] C) CSP + `Permissions-Policy` locking down camera/mic/geolocation (no room needs them)
- [ ] Other: __________________________________________
> USER NOTES:
