...
- [ ] A) All 7 games + all 4 orphan rooms (universal cue coverage)
...
- [ ] A) Full set everywhere it applies
...
- [ ] B) Only on deliberate targets (map pins in atlas, wedge slices in fractures)
- [ ] C) Only during a live timer (clock tick in chronos/gauntlet)
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Yes, for the ceremony rooms only (thread/seance/ladder) — synth still the fallback
- [ ] C) Yes for the room-completion `stinger()` only; interactions stay synth
- [ ] Other: __________________________________________
...
- [ ] B) Themed per mood: séance = eerie, thread = warm/speakeasy, fractures = brittle/glass, atlas = travel
- [ ] C) Themed only for correct/wrong/stinger; place/hover stay generic
- [ ] Other: __________________________________________
...
- [ ] A) Keep current: door+glass+piano → thread/seance/ladder ceremonies only
- [ ] B) Reuse `sfxGlassClink` for fractures slice-lock, `sfxPianoChord` for chronos reveal
...
- [ ] A) Rich pair everywhere (better feel, slightly longer tones)
- [ ] B) Plain pair for fast games (chronos/ignite/gauntlet), rich pair for slow/ceremony rooms
- [ ] C) Let each game pick; document the choice in its component
- [ ] Other: __________________________________________
> USER NOTES (per game):
...
- [ ] C) Per-category ceilings (hover softest, stinger loudest) shared across games
- [ ] Other: __________________________________________
...
- [ ] B) Rate-limit / throttle repeats (min gap ms) per game
...
- [ ] B) Ignite + gauntlet + streak-on-board, cap the pitch so it never gets shrill
...
- [ ] B) Full fanfare everywhere a game can be "won" or "lost"
- [ ] C) Ceremony rooms (thread/seance/ladder) get a bespoke close, others get `stinger()`
- [ ] Other: __________________________________________
...
- [ ] A) No — single global mute silences everything, keep it simple
...
- [ ] B) Ceremony rooms (thread/seance) show a one-time "sound recommended" nudge
...
- [ ] C) Reduced-motion drops decorative cues (hover/combo) but keeps result cues (correct/wrong)
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] A) Any click anywhere (current) — no game-specific handling
- [ ] B) Each game arms audio on its primary action (place tile, spin wedge, start timer)
- [ ] C) A single "tap to enable sound" affordance shared across rooms
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Use sensible defaults — you propose one per game
...
- [ ] A) Nothing — every room may sound
