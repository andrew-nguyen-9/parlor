...
- [ ] B) Framer Motion is the default everywhere for consistency (one mental model, `AnimatePresence` for exits)
- [ ] C) CSS for entrance/idle, Framer only for orchestrated exit/reorder (`AnimatePresence`, layout animation)
...
- [ ] A) mystery (eye-glow), thread (chain links), ladder (rung climb) — all expressible in keyframes
...
- [ ] A) Only clock (gear-train) + map (starfield) + streak (flame) — the three 3D/particle rooms
- [ ] B) Those three plus seance (candle particles / planchette drift)
- [ ] C) Those three plus wedges (shattered-mirror shards)
- [ ] Other: __________________________________________
> USER NOTES (per game if it varies):
...
- [ ] A) Only exits/presence + shared-layout reorder (`AnimatePresence`, `layout`) — CSS does entrances/idle
- [ ] B) Entrances + exits + gesture (drag/tap springs) — Framer is the interaction layer, CSS is decoration
- [ ] C) Anything spring-physics-shaped (planchette, wedge fill, deal-in); CSS only for micro-press
- [ ] Other: __________________________________________
> USER NOTES (per game if it varies):

### Q5: Deck deal-in + card flip-to-blurb (`Deck.tsx`) — engine?
- [ ] A) Keep Framer (stagger + spring feel is the lobby's first impression)
- [ ] B) Move to CSS staggered keyframes — deal-in is deterministic, no gesture needed
- [ ] C) Framer for the flip gesture, CSS for the ambient deal-in
- [ ] Other: __________________________________________
> USER NOTES:

### Q6: Planchette drag (`seance/Planchette.tsx`) — spring engine?
- [ ] A) Framer drag + spring (natural weight; already there)
- [ ] B) Hand-rolled RAF spring (drop the Framer dep from this leaf if it's the only user)
- [ ] C) CSS transition on pointer-move (cheapest; loses momentum)
- [ ] Other: __________________________________________
> USER NOTES:

### Q7: Wedges shattered-mirror ring fill (`WedgesGame.tsx`) — engine?
- [ ] A) Framer per-shard spring (staggered wedge-by-wedge fill)
- [ ] B) CSS keyframe fill per wedge (no per-shard physics)
- [ ] C) Canvas/RAF shard simulation (true shatter, heavier)
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Whole play surface in Three.js (cogs + dial + flap as textured meshes)
- [ ] C) Cogs in Three.js only when supported; CSS-SVG gear fallback is the primary and WebGL is the upgrade
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Replace Phaser with hand-rolled RAF canvas (drop a heavy dep for one room)
...
- [ ] A) Stars in Three.js, but tap-targets + numbered chip strip stay DOM (raycast only picks)
- [ ] B) Everything (stars + selection gl/labels) inside the WebGL canvas
- [ ] C) Stars as a static/RAF-drift canvas layer, selection entirely DOM on top
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Diverge — starfield and gear-train have little in common; duplicate is cheaper than abstraction
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Streak's candle + Séance's planchette-follows-pointer (two, both diegetic)
...
- [ ] A) Resolve to the centered default (no tracking); pointer enhances only where `hover:hover`
- [ ] B) Touch drives it via last-tap position (glow follows taps)
...
- [ ] A) The canvas owns the single looping animation; Framer is finite-only (entrances/exits ≤600ms) there
- [ ] B) Budget is per-engine — canvas loop + one Framer loop is still "one per viewport" if visually merged
...
- [ ] A) CSS 3D transform (`rotateY`, `transform-style: preserve-3d`) — no JS motion lib needed
- [ ] B) Framer (spring flip + `AnimatePresence` for the reveal content)
- [ ] C) CSS flip, Framer only for the daily board's deal-in stagger
- [ ] Other: __________________________________________
> USER NOTES:

### Q16: Gauntlet (`GauntletGame.tsx`) — it embeds a `WorldMap` pin-drop; motion engine for the run flow?
- [ ] A) Framer for round-to-round transitions; the pin-drop map stays its own offline polygon render
- [ ] B) CSS transitions between rounds (each round is a discrete screen swap)
- [ ] C) Framer transitions + a shared results "line of squares" reveal
- [ ] Other: __________________________________________
> USER NOTES:
...
- [ ] B) Both Framer (`AnimatePresence` staggered children) — reuse one reveal primitive
...
- [ ] A) Keep hand-rolled RAF (tiny, zero dep, easy to gate on reduced-motion)
- [ ] B) Fold into the Phaser/Three layer where a room already has one running
...
- [ ] A) Framer owns all cross-room overlays (consistent enter/exit spring)
...
- [ ] A) Yes — CSS/RAF it so that route ships no Framer (per-route bundle wins)
...
- [ ] C) Both — table for intent + automated check for drift
- [ ] Other: __________________________________________
> USER NOTES:

### Q22: Reduced-motion static frame — who authors it per room, and is it engine-specific?
- [ ] A) Each room ships a designed static frame first (lusion rule); engine just skips the loop
- [ ] B) Global `globals.css` kill-list is enough; no per-room static art
- [ ] C) WebGL rooms get a hand-authored static poster; CSS rooms rely on the kill-list
- [ ] Other: __________________________________________
> USER NOTES (per game if it varies):
