...
- [x] C) A single "tonight's sky" title card (the `skyRegion` flavor) that dissolves into the field
...
- [x] C) Always eight for more deduction surface
- [ ] Other: __________________________________________
> USER NOTES: This should feel like a user is searching through the a constellation map.
...
- [x] C) Ring / arc the player's eye sweeps across
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Drag-to-orbit the whole field, then tap
- [ ] Other: __________________________________________
> USER NOTES:

### Q8: How central is the "rule out this one" elimination move?
- [ ] A) Core loop — encourage ruling out until one survives (current)
- [x] B) Optional aid; a confident player can name immediately
- [ ] C) Auto rule-out: tapping an omen greys every figure it forbids
- [ ] Other: __________________________________________
> USER NOTES:

### Q9: When a player rules a figure out, what confirms it?
- [x] A) Figure dims in 3D + chip greys (current)
- [x] B) Above, plus a soft "cross-out" mark on the chip
- [x] C) It drifts to the back of the field, out of the way
- [ ] Other: __________________________________________
> USER NOTES:

### Q10: Can a ruled-out figure be restored?
- [x] A) Yes, freely, via "restore this one" (current)
- [x] B) Yes, but each restore costs points
- [ ] C) No — ruling out is a committed decision
- [ ] Other: __________________________________________
> USER NOTES:

### Q11: Which structural omen KINDS should stay in the rotation?
- [x] A) All current kinds: count, parity, line count, closed loop, hub star, connected pieces, brightest-star position
- [x] B) Trim to the most intuitive (count, loop, brightest-star) for accessibility
- [x] C) Add new structural kinds (symmetry, longest single line, star nearest center) — specify below
- [ ] Other: __________________________________________
> USER NOTES: Add all but make sure they are accessible

### Q12: How should an omen be phrased to the player?
- [x] A) Plain imperative fact ("The pattern has an odd number of stars.") — current
...
- [x] B) Only after a wrong guess, as a nudge
- [x] C) Optional "assist mode" the player opts into
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] B) Fewer, gentler penalties (−15 per wrong) so a slip isn't near-fatal
- [ ] C) Flat: solved-first-try vs not, no partial score
- [ ] Other: __________________________________________
> USER NOTES:

### Q15: Is a wrong guess penalized twice (points AND auto rule-out)?
- [x] A) Yes — wrong guess costs points and rules that figure out (current)
...
- [x] A) 100 = first-guess solve; the share mark 🟩 already reflects zero-wrong
- [ ] B) Any solve under 3 guesses (🟨 band)
- [x] C) Introduce named tiers (e.g. "Clear Sky" / "Overcast")
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Optional speed bonus for players who want it
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Solve within the day's allotted guesses (see win/lose)
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] A) Name the one figure satisfying every omen (current)
...
- [x] B) Capped guesses (e.g. 3); running out ends the round unsolved
- [x] C) Soft fail: after N wrong, the answer is revealed but streak breaks
- [ ] Other: __________________________________________
> USER NOTES:

### Q21: What does the winning reveal look like?
- [ ] A) Figure name banner + confetti + calm stinger swell (current)
- [x] B) Solution figure brightens, its lines re-trace, THEN the name — earned reveal (spec)
- [x] C) Above, plus the other five dim to near-black
- [ ] Other: __________________________________________
> USER NOTES:

### Q22: After solving, should the puzzle stay interactive?
- [ ] A) Locked win screen with the name + score + share (current)
- [x] B) Free-look: player can orbit the solved figure before sharing
- [x] C) Auto-advance to an educational card about the real constellation
- [ ] Other: __________________________________________
> USER NOTES:

### Q23: What should the shared result string convey?
- [ ] A) Date, 🟩/🟨/⬛ mark, guesses, score, pattern name (current)
- [x] B) Above minus the pattern name (avoid spoiling the day's answer)
...
- [x] A) Focus → (rule out) → name, repeat freely until solved (current)
...
- [x] A) Daily only, plus an archive of past days (spec: Daily + Archive)
...
- [x] C) Both: replay for fun + a permanent solved-constellation collection
- [ ] Other: __________________________________________
> USER NOTES:

### Q27: Should there be any hint system, and how restrained?
- [ ] A) None — pure deduction (current build has no hints)
- [x] B) One late, opt-in hint: dim one clearly-wrong figure (costs points)
- [x] C) Progressive: shimmer → highlight an omen → mark a decoy (spec §Hints)
- [ ] Other: __________________________________________
> USER NOTES:

### Q28: When the day rolls over, what happens to an unfinished round?
- [x] A) It's replaced by the new day's puzzle; yesterday moves to archive
- [ ] B) Yesterday stays resumable until solved, streak-neutral
- [x] C) Auto-marked unsolved (breaks streak) and archived
- [ ] Other: __________________________________________
> USER NOTES:

### Q29: Should progress persist if the player leaves mid-round?
- [x] A) Yes — restore selection + ruled-out set from localStorage
...
- [x] A) Monday easy → Sunday master, per Parlor cadence (spec)
...
- [x] A) Number of omens needed to isolate the answer
- [ ] B) How similar the six figures are (near-identical star counts / shapes)
- [x] C) Subtlety of the discriminating omen (e.g. brightest-star position vs raw count)
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] C) Mixed by weekday: loose early, near-twins on the weekend
- [ ] Other: __________________________________________
> USER NOTES:

### Q33: Should harder days use harder-to-read omen KINDS?
- [x] A) Yes — early week leans count/loop; weekend leans hub/components/brightest
- [x] B) All kinds available any day; difficulty comes from figure similarity
- [ ] C) Restrict weekend to only the two subtlest kinds
- [ ] Other: __________________________________________
> USER NOTES:

### Q34: Should figure complexity (star/line count) grow with difficulty?
- [x] A) Yes — denser figures with more lines on harder days (spec: dense fields)
- [ ] B) Cap complexity for legibility; difficulty from omens only
- [x] C) Grow star count but keep lines sparse so figures stay readable
- [ ] Other: __________________________________________
> USER NOTES:

### Q35: Should the generator guarantee a UNIQUE answer every day?
- [x] A) Yes, always — validate that exactly one figure survives all omens (non-negotiable)
...
- [x] A) Calm luxury planetarium — deep space, glass panels, quiet motion (spec)
...
- [x] B) Shift to a cooler star-blue for a colder sky
...
- [x] A) Gentle bob + micro-rotation, whole-sky twinkle (current)
...
- [x] A) Selected brightens to blue-white + aura; others stay normal (current)
...
- [x] C) Omens overlaid as floating glass cards in the scene
- [ ] Other: __________________________________________
> USER NOTES:
...
- [x] B) Layered nebula / dust depth for richness (spec §Background)
- [ ] C) Near-black void; figures float with almost no backdrop
- [ ] Other: __________________________________________
> USER NOTES: Background should not make the constellations harder to see. The constellations should stand out enough to survive being above the background.

### Q42: How loud should audio be?
- [x] A) Minimal: ambient bed, tiny focus cues, restrained solve stinger (current)
...
- [x] A) Numbered chips + omens fully play the puzzle in the DOM (current guarantee)
- [x] B) Above, plus a flat 2D SVG rendering of the six figures
...
- [x] B) Still scene but keep a single gentle solve fade
- [ ] C) Skip the 3D entirely; render the 2D fallback
- [ ] Other: __________________________________________
> USER NOTES:

### Q45: When an archived date has no puzzle row (DB connected, empty), what shows?
- [x] A) "No star chart survives for that night" flavor empty-state (current)
...
- [x] A) Full playable puzzle from the committed `star-catalog.json`, deterministic (CLAUDE.md rule)
- [x] B) Above, and identical to what an online player sees that day
- [ ] C) A reduced "practice" puzzle only
- [ ] Other: __________________________________________
> USER NOTES:

### Q47: What if the player rules out all six figures (including the answer)?
- [x] A) Confirm is disabled; a nudge invites restoring one (deduction slipped)
...
- [x] A) Field compact on top, omens collapsed, chips + actions thumb-reachable (current)
